...
// so Calendar features are silently skipped. When a token doesn't exist yet,
...
func Init(ctx context.Context, credentialsPath, tokenPath string, authIO *AuthIO) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			logger.Log.Warnf("Calendar credentials file %q not found — Calendar features disabled", credentialsPath)
			return nil
		}
		return fmt.Errorf("read credentials: %w", err)
	}

	config, err := google.ConfigFromJSON(creds, cal.CalendarScope)
	if err != nil {
	...
	if err != nil {
		...
		if err := saveToken(tokenPath, tok); err != nil {
			logger.Log.Warnf("Failed to save calendar token: %v", err)
		}
	}
